...
namespace Application.Recipes.Commands.CreateRecipe
{
    public class CreateRecipeCommandHandler : IRequestHandler<CreateRecipeCommand, Recipe>
    {
        ...
        public async Task<Recipe> Handle(CreateRecipeCommand request, CancellationToken cancellationToken)
        {
            ...
            var recipe = new Recipe() { RecipeName = request.Name, RecipeDescription = request.Description, RecipeCategory = category, Status = request.Status, UserId = request.UserId ,Ingredients = new List<IngredientQuantity>(), RecipeCoverImage = request.RecipeCoverImage };
